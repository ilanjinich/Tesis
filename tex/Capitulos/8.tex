...
\BlankLine
\While{ Existe un aspirante $a$ que no esta emparejado y que no haya sido rechazado por todas las universidades.}{
...
El algoritmo orientado a los aplicantes sigue de acuerdo a los siguientes pasos \footnote{Se modifico un poco el orden con respecto al código para efectos de la explicación}.
\begin{figure}[H]
\begin{minipage}{.5\linewidth}
...
$a_10$ solicita a $c_5$, $c_5$ acepta a $a_{10}$ y rechaza a $a_5$ porque $\{c_4,c_5\}$ queda lleno. $a_5$ solicita a $c_3$ y es aceptada. El algoritmo finaliza.
